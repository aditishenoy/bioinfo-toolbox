...
def write_pdb_atm_record(atom):
    print(
        "{:6s}{:5d} {:^4s}{:1s}{:3s} {:1s}{:4d}{:1s}   {:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}".format(
            "ATOM",
            atom["atm_no"],
            atom["atm_name"],
            " ",
...
def get_coordinates(pdbfile, chain):
    ...
    if not chain:
        ...
        with open(pdbfile, "r") as pdb_filename:
            pdb_filename.seek(0)

    with open(pdbfile, "r") as pdb_filename:
        for line in pdb_filename:
...
def get_cb_coordinates(pdbfile, chain):
    ...
    # need to sort to get the sequence correct
    # keys() generate a list of keys and then sorted() sort the keys
    sorted_keys = sorted(res_dict.keys())
     
    for i in sorted_keys:

        # If residue dictionary (res_dict) has carbon-beta AND carbon-alpha coordinates
        if len(res_dict[i]) > 1:
            tmp_i += 1
            cb_lst.append(res_dict[i][-1])

        # If residue dictionary (res_dict) has only carbon-alpha atom coordinate as in Glycine (GLY)
        elif len(res_dict[i]) == 1:
            tmp_i += 1
            cb_lst.append(res_dict[i][0])
            # print tmp_i,i,res_dict[i][0],res_dict[i][-1]
...
def get_atom_seq(pdbfile, chain="", model=1, return_lines=False):
    ...
    with open(pdbfile, "r") as pdb_filename:
        ...
        line_lst = [l[1] for l in sorted(line_dict.items(), key=operator.itemgetter(0))]
